...
在阅读 [OpenGL ES 着色器语言规范][url-9]（PDF）时，我产生了这个项目想法。我一直想做一个粒子系统，但我卡在如何绘制粒子的问题上。表示位置的纹理数据需要以某种方式作为顶点反馈到管道中。通常，[缓冲区纹理][url-10]——由数组缓冲区支持的纹理——或[像素缓冲区对象][url-11]——异步纹理数据复制——可用于此操作，但 WebGL 没有这些功能。从 GPU 中提取纹理数据，并将其作为每帧上的数组缓冲区重新加载是不可能的。
...
然而，有一个潜在的问题：允许实现将顶点着色器纹理绑定的数量限制为 0（GL_MAX_vertex_texture_IMAGE_UNITS）。所以从技术上讲，顶点着色器必须始终支持 texture2D ，但它们不需要支持实际的纹理。这有点像飞机上不载客的餐饮服务。这些平台不支持这种技术。到目前为止，我只在一些移动设备上遇到过这个问题。
...
障碍物纹理上的颜色是曲面法线。也就是说，每个像素都有一个指向它的方向，一个将粒子导向某个方向的流。空隙有一个特殊的常值（0，0）。这不是标准化的（长度不为1），因此它是对粒子没有影响的带外值。
