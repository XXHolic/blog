...
原因分析：
- Mac 中，Git 还有一种 “osxkeychain” 模式，它会将凭证缓存到你系统用户的钥匙串中。 这种方式将凭证存放在磁盘中，并且永不过期，但是是被加密的。
- 在 Mac 的钥匙串中，可以发现匹配 GitHub 相关的钥匙串。


在 credential 的信息中，有个 host 的属性，默认匹配的方式只匹配 host，所以这里只要是 https://github.com 前缀的都会符合，但现在的的情况是不同的库是不同的帐号，所以要让 credential 也匹配剩下的路径，执行下面的指令：
```bash
git config credential.useHttpPath 'true'
```
执行后再次提交就会出现输入帐号和密码的提示，输入一次后，会在 Mac 中存下来。

如果上面的方式不行，看看这个文档，可能会有帮助。
