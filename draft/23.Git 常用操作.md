...
git status
```
这个命令会显示所有文件所处于的状态。这种方式会显示比较详细的信息。如果想要简化，可以使用下面的指令：
```shell
git status -s
```
...
git branch feature
```
新建 feature 本地分支，但还是停留在当前分支，并切换到 feature 分支。<br />
4、删除分支
```shell
...
git merge feature
```
把 feature 分支的 commit 内容合并到当前分支。默认情况下，当合并采用的是 fast-forward 策略，那么就不会产生合并 commit。不推荐这么做，这样子追溯的时候就太方便。如果需要产生合并记录，则加上另外的参数：
```shell
