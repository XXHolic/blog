...
在初次配置 Git 的时候，配置了 global 帐号，也设置了存储凭证的方式为 `store`。使用 https 方式克隆另外一个帐号的项目，提交的时候，默认使用了全局的帐号用户名，导致无法提交，查询了资料后，发现了两种解决问题的方式：
- 使用 SSH 方式克隆库，添加对应的 SSH Key，这样就不会冲突了。
- https 方式配置 credential 。

由于官方推荐使用 https ，在此实践的是后面的一种方法。
...
最后输入完成，回车两次就会出现相应的提示。

再次提交，出现下面输入提示。输入即可。

比较奇怪的是那个错误提示，但实际上配置生效了。
