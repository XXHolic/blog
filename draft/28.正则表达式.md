...
### 思考
上面是基于写好的正则进行拆分，这个时候不妨尝试倒着顺序看看，如果一开始不告知功能需求，只提供这些拆分的信息，也应该是可以推断出对应的功能需求。例如提供如下的信息，请推测出对应可能实现的正则功能。

- `/ /`，是创建正则的形式。
- `\s`，是表示空字符串的元字符，属于普通字符。
- `g`，是修饰符，表示执行全局匹配。

很明显吧？至少可以猜到的功能是：匹配所有的空字符串。
