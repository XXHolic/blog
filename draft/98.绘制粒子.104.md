...
```
粒子的颜色信息会存在纹理中，这里创建了宽高相等的纹理，每个粒子颜色 RGBA 4 个分量，每个分量 8 位。注意这里生产随机颜色分量的大小范围是 [0, 256) 。
...
gl_Position = vec4(2.0 * v_particle_pos.x - 1.0, 1.0 - 2.0 * v_particle_pos.y, 0, 1);
```
`gl_Position` 变量顶点转换到裁剪空间中的坐标值，裁减空间范围 [-1.0, +1.0] ，想要显示就必须要在这个范围内，这里的计算方式也是为了这个目的。
...
### 小结
经过了上面的详细逻辑后，再回头看看一开始的主要思路，换个方式表述一下：
...
```
在前面的具体解释中有说这样的计算方式确保了得到的坐标都在 0 到 1 之间，但能生成这个范围内的方式应该不止这一种，为什么偏偏选这种，个人也不太清楚。后面片元着色器中计算最终采用位置 ramp_pos 时也用了这样类似的方式。
