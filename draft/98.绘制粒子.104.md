...
在 WebGL 基础教程里面每个顶点是个三维坐标，但这里直接就是一个数字索引，这个需要看顶点着色器的具体实现。
...
```
从上面各个地方对应的逻辑，着色器中变量对应的实际值：
- `a_index` ： 是 `particleIndices` 里面的粒子索引数据。
- `u_particles` ：是所有粒子颜色信息的纹理 `particleStateTexture` 。
- `u_particles_res` ： 是纹理 `particleStateTexture` 的宽高 `particleStateResolution` ，也是总粒子数的平方根，也是粒子索引数据长度的平方根。
...
```c
  v_particle_pos=vec2(
          ...
          color.g / 255.0 + color.a);
```
源码注释说“从像素的 RGBA 值解码当前粒子位置”，结合前面随机生成的粒子数据来看，这样的计算方式保证了这个位置坐标分量在 0 - 256 之间。

```c
gl_Position = vec4(2.0 * v_particle_pos.x - 1.0, 1.0 - 2.0 * v_particle_pos.y, 0, 1);
```
`gl_Position` 变量顶点转换到裁剪空间中的坐标值，裁减空间范围 -1.0 到 +1.0，想要显示就必须要在这个范围内。
